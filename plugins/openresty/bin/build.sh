...
export _RESTY_CONFIG_DEPS="--with-pcre \
    --with-cc-opt='-DNGX_LUA_ABORT_AT_PANIC -O2 -DTCP_FASTOPEN=23' \
    --with-ld-opt='' \
    "
...
if [ $# -eq 0 ]; then
	_init
	_openresty /root/.asdf/plugins/openresty/bin /root/.asdf/installs/openresty/v1.21.4.1 all
else
	$@
fi
